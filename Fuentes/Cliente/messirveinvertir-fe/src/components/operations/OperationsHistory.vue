<template>
  <ion-page>
    <ion-toolbar>
      ...
      <ion-title class="c-title"> {{ $t('operations.operations_history') }} </ion-title>
    </ion-toolbar>
    <ion-content class="ion-padding">
     <TheButton  href="operation-creator" expand="block" inner-text="operations.new_operation" />
     <ion-row class="ion-text-center ion-margin-top ion-margin-bottom">
         <ion-col>
            <ion-chip :disabled="noItems" @click="filterByPeriod('1_day')" color="tertiary">
              <ion-label class="font-bold"  color="tertiary">{{ $t('operations.time_filter.1_day') }}</ion-label>
            </ion-chip>
         </ion-col>
         <ion-col>
            <ion-chip :disabled="noItems" @click="filterByPeriod('1_week')" color="tertiary">
              <ion-label class="font-bold" color="tertiary">{{ $t('operations.time_filter.1_week') }}</ion-label>
            </ion-chip>
         </ion-col>
         <ion-col>
            <ion-chip :disabled="noItems" @click="filterByPeriod('1_month')" color="tertiary">
              <ion-label class="font-bold" color="tertiary">{{ $t('operations.time_filter.1_month') }}</ion-label>
            </ion-chip>
         </ion-col>
         <ion-col>
            <ion-chip :disabled="noItems" @click="filterByPeriod('3_months')" color="tertiary">
              <ion-label class="font-bold" color="tertiary">{{ $t('operations.time_filter.3_months') }}</ion-label>
            </ion-chip>         
          </ion-col>
         <ion-col>
            <ion-chip :disabled="noItems" @click="filterByPeriod('1_year')" color="tertiary">
              <ion-label class="font-bold" color="tertiary">{{ $t('operations.time_filter.1_year') }}</ion-label>
            </ion-chip>
         </ion-col>
     </ion-row>
     <template  v-if="items">
       <template v-for="(item, index) in items" :key="index">
          <ion-item-divider class="divider ion-padding-top ion-padding-bottom"
           v-if="index == 0">
            <ion-label>
          ...
          <TheOperationItem
            ...
            />
          <ion-item-divider class="divider ion-padding-top ion-padding-bottom"
           v-if="items.length > index+1 && dateLabel(items[index].fecha) != dateLabel(items[index+1].fecha)">
            <ion-label>
              <span v-if="items[index+1]">{{ dateLabel(items[index+1].fecha) }}</span>
            </ion-label>
          </ion-item-divider>
       </template>   
     </template>
